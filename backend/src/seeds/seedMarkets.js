...
// Force sync enabled for seeding regardless of env setting
...
// Full category list matching Polymarket (Trending/Breaking/New/Mentions are nav-only virtual tabs)
const categories = [
  { name: 'Politics',    slug: 'politics',    icon: 'landmark',    description: 'Political prediction markets',          order: 1 },
  { name: 'Sports',      slug: 'sports',      icon: 'trophy',      description: 'Sports prediction markets',             order: 2 },
  { name: 'Crypto',      slug: 'crypto',      icon: 'bitcoin',     description: 'Cryptocurrency prediction markets',     order: 3 },
  { name: 'Esports',     slug: 'esports',     icon: 'gamepad-2',   description: 'Esports prediction markets',            order: 4 },
  { name: 'Iran',        slug: 'iran',        icon: 'globe-2',     description: 'Iran news prediction markets',          order: 5 },
  { name: 'Finance',     slug: 'finance',     icon: 'trending-up', description: 'Financial prediction markets',          order: 6 },
  { name: 'Geopolitics', slug: 'geopolitics', icon: 'earth',       description: 'Global geopolitics prediction markets', order: 7 },
  { name: 'Tech',        slug: 'tech',        icon: 'cpu',         description: 'Technology prediction markets',         order: 8 },
  { name: 'Culture',     slug: 'culture',     icon: 'music',       description: 'Culture & entertainment markets',       order: 9 },
  { name: 'Economy',     slug: 'economy',     icon: 'bar-chart-2', description: 'Economy prediction markets',            order: 10 },
  { name: 'Weather',     slug: 'weather',     icon: 'cloud-sun',   description: 'Weather prediction markets',            order: 11 },
  { name: 'Elections',   slug: 'elections',   icon: 'vote',        description: 'Election prediction markets',           order: 12 },
];
...
const seed = async () => {
  ...
  // 1. Wipe all mock markets (those without a real conditionId from Polymarket)
  const deletedMocks = await Market.deleteMany({ conditionId: null });
  console.log(`Deleted ${deletedMocks.deletedCount} mock markets`);

  // 2. Upsert full category list (12 real categories matching Polymarket nav)
  ...
  // Remove old categories that are no longer in our list (e.g. 'news')
  const validSlugs = categories.map(c => c.slug);
  const removed = await Category.deleteMany({ slug: { $nin: validSlugs } });
  if (removed.deletedCount) console.log(`Removed ${removed.deletedCount} obsolete categories`);
  console.log(`Upserted ${categories.length} categories`);

  // 3. Sync real markets from Polymarket Gamma API (includes trimToCategoryLimit)
  console.log('Fetching live markets from Polymarket...');
  const result = await runSync();
  console.log(`\nSeeding complete! +${result.created} new, ~${result.updated} updated, ${result.trimmed} trimmed to 50/category cap`);
  process.exit(0);
};
