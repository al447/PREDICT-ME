...
(async () => {
  ...
  const tokenAddress = process.env.WITHDRAW_TOKEN_ADDRESS || process.env.MOCK_USDC_ADDRESS;
  ...
  if (Number(tokenFmt) < 1) {
    console.warn(`⚠️  Platform wallet has < 1 ${symbol}. Mint MockUSDC via faucet() function or fund the wallet.`);
  } else {
