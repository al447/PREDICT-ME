...
const magicAuth = async (req, res, next) => {
  try {
    ...
    // Verify the DID token signature + claims
    try {
      magicAdmin.token.validate(didToken);
    } catch (e) {
      return res.status(401).json({ success: false, error: 'Invalid or expired Magic token' });
    }

    // Validate DID token issuer — must be a did:ethr address from Magic infrastructure
    try {
      const [, claim] = magicAdmin.token.decode(didToken);
      if (!claim?.iss || !claim.iss.startsWith('did:ethr:')) {
        return res.status(401).json({ success: false, error: 'Magic token issuer invalid' });
      }
    } catch (e) {
      return res.status(401).json({ success: false, error: 'Magic token could not be decoded' });
    }
