...
router.get('/orderbook/:marketSlug', async (req, res, next) => {
  try {
    ...
    // Phase 1I: Markets with on-chain tokens use PredictMe's own CLOB
    // Only fall back to Polymarket for markets without on-chain tokens
    const hasOnChainTokens = market.token0 && market.token1;
    if (hasOnChainTokens) {
      return res.status(503).json({
        success: false,
        error: 'Use /api/clob/orderbook for on-chain markets',
        source: 'unavailable',
        marketSlug,
        asks: [],
        bids: [],
    ...
    // Fetch from Polymarket CLOB (legacy fallback only)
    const response = await fetch(`${CLOB_BASE}/book?token_id=${market.polymarketTokenId}`);
    if (!response.ok) {
