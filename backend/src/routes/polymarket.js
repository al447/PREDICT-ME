...
// ── Social proxy routes ───────────────────────────────────────────────────────
// These fetch read-only social/liquidity data from Polymarket's public API
// and serve it to our frontend so we have social proof without building our own.

const POLYMARKET_WEB_API = 'https://polymarket.com/api';
...
async function safeFetch(url, opts = {}) {
  ...
  try {
    const res = await fetch(url, { ...opts, signal: controller.signal, headers: { Accept: 'application/json', 'User-Agent': 'Mozilla/5.0' } });
    clearTimeout(timer);
...
// GET /api/polymarket/social/comments?eventSlug=<slug>&limit=20
router.get('/social/comments', async (req, res, next) => {
  try {
    const { eventSlug, limit = 20, offset = 0 } = req.query;
    if (!eventSlug) return res.status(400).json({ error: 'eventSlug required' });

    const url = `${GAMMA_API}/comments?event_slug=${encodeURIComponent(eventSlug)}&limit=${limit}&offset=${offset}`;
    const data = await safeFetch(url);
    if (!data) return res.json({ comments: [], total: 0, source: 'unavailable' });
    res.json({ comments: Array.isArray(data) ? data : (data.comments || []), total: data.total || 0, source: 'polymarket' });
  } catch (error) {
    next(error);
  }
});

// GET /api/polymarket/social/holders?conditionId=<id>&limit=10
router.get('/social/holders', async (req, res, next) => {
  try {
    const { conditionId, limit = 10 } = req.query;
    if (!conditionId) return res.status(400).json({ error: 'conditionId required' });

    const url = `${GAMMA_API}/top-holders?condition_id=${encodeURIComponent(conditionId)}&limit=${limit}`;
    const data = await safeFetch(url);
    if (!data) return res.json({ holders: [], source: 'unavailable' });
    res.json({ holders: Array.isArray(data) ? data : (data.holders || []), source: 'polymarket' });
  } catch (error) {
    next(error);
  }
});

// GET /api/polymarket/social/activity?conditionId=<id>&limit=20
router.get('/social/activity', async (req, res, next) => {
  try {
    const { conditionId, limit = 20, offset = 0 } = req.query;
    if (!conditionId) return res.status(400).json({ error: 'conditionId required' });

    const url = `${GAMMA_API}/activity?condition_id=${encodeURIComponent(conditionId)}&limit=${limit}&offset=${offset}`;
    const data = await safeFetch(url);
    if (!data) return res.json({ activities: [], source: 'unavailable' });
    res.json({ activities: Array.isArray(data) ? data : (data.activities || []), source: 'polymarket' });
  } catch (error) {
    next(error);
  }
});
