...
contract M1FullIntegrationTest is Test {
    ...
    ICTFFull public ctf;
    IExchangeFull public ctfExchange;
    IUmaAdapterTest public umaAdapter;
    ...
    function setUp() public {
        ...
        ctf = ICTFFull(deployCTF());
        
        // Deploy exchanges
        ctfExchange = IExchangeFull(deployCTFExchange(address(ctf), address(usdc)));
        negRiskExchange = INegRiskCtfExchange(deployNegRiskExchange(address(ctf), address(usdc)));
        
        // Deploy adapters
        umaAdapter = IUmaAdapterTest(deployUmaAdapter(address(ctf)));
        negRiskAdapter = INegRiskAdapter(deployNegRiskAdapter(address(ctf), address(usdc)));
        
        // Deploy WalletFactory (placeholder Safe infra addresses for local skeleton)
        walletFactory = new WalletFactory(
            address(0x1010101010101010101010101010101010101010), // proxyFactory
            address(0x2020202020202020202020202020202020202020), // safeSingleton
            address(0x3030303030303030303030303030303030303030), // fallbackHandler
            deployer                                              // admin
        );
    ...
    function test_ClobTradeExecution() public {
        ...
        IExchangeFull.Order[] memory orders = new IExchangeFull.Order[](1);
        orders[0] = IExchangeFull.Order({
            maker: alice,
            taker: bob,
    ...
    function test_UmaResolutionFlow() public {
        ...
        vm.mockCall(
            address(ctf),
            abi.encodeWithSelector(ICTFFull.payoutNumerators.selector, conditionId, 0),
            abi.encode(1)
        );
        vm.mockCall(
            address(ctf),
            abi.encodeWithSelector(ICTFFull.payoutNumerators.selector, conditionId, 1),
            abi.encode(0)
        );
        vm.mockCall(
            address(ctf),
            abi.encodeWithSelector(ICTFFull.payoutDenominator.selector, conditionId),
            abi.encode(1)
        );
    ...
    function test_WalletFactory() public {
        ...
        vm.prank(alice);
        address createdWallet = walletFactory.getOrCreateProxy(alice);
...
interface INegRiskCtfExchange {
    function addAdmin(address admin) external;
    function matchOrders(IExchangeFull.Order[] calldata orders) external;
}
