...
/**
 * Build props for MoonPayBuyWidget from a session API response.
 * @param {Object} session - Response from POST /api/deposits/moonpay/session
 * @param {Function} onSignUrl - async (url) => signature  — calls backend sign-url
 * @returns {Object} props ready to spread onto <MoonPayBuyWidget>
 */
export const buildWidgetParams = (session, onSignUrl) => ({
  variant: 'overlay',
  baseCurrencyCode: session.baseCurrencyCode || 'usd',
  baseCurrencyAmount: String(session.baseCurrencyAmount),
  currencyCode: session.currencyCode || MOONPAY_DEFAULT_CURRENCY,
  walletAddress: session.walletAddress,
  paymentMethod: session.paymentMethod,
  externalTransactionId: session.externalTxId,
  showWalletAddressForm: false,
  onUrlSignatureRequested: onSignUrl,
});
