    ...
    <div className="mb-2">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 flex items-center justify-between px-3 py-2 rounded-lg bg-[var(--color-green)]/75 border border-[var(--color-green)] hover:bg-[var(--color-green)] active:scale-[0.98] transition-all"
        >
          <span className="text-[12px] font-bold text-white">{yes?.name ?? 'Yes'}</span>
          <span className="text-[11px] text-white/80">{yes?.price ? `${yes.price}¢` : ''}</span>
        </button>
        <button
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
          className="flex-1 flex items-center justify-between px-3 py-2 rounded-lg bg-[var(--color-red)]/75 border border-[var(--color-red)] hover:bg-[var(--color-red)] active:scale-[0.98] transition-all"
        >
          <span className="text-[12px] font-bold text-white">{no?.name ?? 'No'}</span>
          <span className="text-[11px] text-white/80">{no?.price ? `${no.price}¢` : ''}</span>
        </button>
      </div>
    </div>
  );
...
const MarketCard = ({ market, className = '' }) => {
  ...
  return (
    <div
      className={`bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl overflow-hidden hover:border-[var(--color-text-muted)] transition-colors group ${className}`}
    >
      <Link to={`/market/${market.slug}`} className="block p-4 pb-3">
        {/* Header: icon + title */}
        <div className="flex items-start gap-3 mb-3">
          <MarketIcon market={market} />
          <h3 className="flex-1 text-[14px] font-medium text-[var(--color-text)] leading-snug line-clamp-2 group-hover:text-[var(--color-gold)] transition-colors">
            {market.title}
          </h3>
        </div>
        ...
        {isBinary ? (
          ...
          <div className="space-y-0 mb-1 divide-y divide-[var(--color-border)]">
