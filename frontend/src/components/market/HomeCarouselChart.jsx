...
const HomeCarouselChart = ({ data, lines, isCrypto, target }) => {
        ...
        {lines.map((line) => {
          ...
          return (
            <div key={line.key} className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-semibold" style={{ color: line.color }}>{isCrypto ? fmtUsd(v) : `${v}%`}</span>
            </div>
          );
        })}
      </div>
