...
const MarketChart = ({ market }) => {
  ...
  const { data: cryptoData, isLoading: cryptoLoading, error: cryptoError } = useQuery({
    queryKey: ['market-crypto-price-history', slug, days],
    queryFn: async () => {
      if (!slug) return null;
      try {
        const res = await marketsAPI.getCryptoPriceHistory(slug, days);
        if (!res.data?.isCrypto) {
          console.log('[MarketChart] Not a crypto market:', slug);
          return null;
        }
        if (!res.data?.chartData?.length) {
          console.warn('[MarketChart] Crypto price data unavailable for', slug, '- message:', res.data?.message);
          return { ...res.data, _unavailable: true };
        }
        return res.data;
      } catch (err) {
  ...
  const chartData = isCryptoChart
    ? cryptoData.chartData
    : (realData?.chartData?.length > 1 ? realData.chartData : fallbackData);
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {isCryptoUnavailable && (
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 font-medium" title="Price API rate limited - showing odds instead">PRICE UNAVAILABLE</span>
            )}
            <p className="text-xs text-[var(--color-text-muted)]">
              {isCryptoChart ? `${cryptoData.symbol}/USD live spot` : (hasRealData ? 'Live market data' : 'Simulated data')} · {timeRange}
            </p>
          </div>
        </div>
          ...
          <LineChart data={chartData} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
            ...
            <YAxis
              domain={isCryptoChart
                ...
                : [Math.max(0, minVal - 5), Math.min(100, maxVal + 5)]}
              tick={{ fill: 'var(--color-text-muted)', fontSize: 10 }}
              tickLine={false}
              axisLine={false}
