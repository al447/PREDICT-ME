...
// Proxy wallet address — deposit destination for the current user.
// Reads from user.smartWallet.proxy (set by backend walletService).
function getProxyDepositAddress(user) {
...
const DepositPage = () => {
  ...
  // Fetch on-chain USDC balance using MetaMask's actual address
  useEffect(() => {
    ...
    return (
      <Layout>
        <div className="max-w-lg mx-auto px-4 py-20 text-center">
          ...
          <p className="text-[var(--color-text-muted)] mb-6">Connect your wallet to deposit USDC.</p>
          <button
  ...
  const handleDeposit = async () => {
    ...
    // Resolve deposit destination: user's proxy wallet (non-custodial)
    const proxyAddress = getProxyDepositAddress(user);
    if (!proxyAddress) {
      toast.error('Your deposit wallet is not ready yet. Please refresh and try again.');
      return;
    }
    ...
    try {
      ...
      // 4. Transfer USDC to user's proxy wallet on-chain (non-custodial)
      toast.loading('Confirm transfer in MetaMask...', { id: 'deposit-tx' });
      const tx = await usdc.transfer(proxyAddress, amountWei);
    ...
    <Layout>
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-[var(--color-text-muted)] mt-1">Add funds to your PredictMe balance</p>
        </div>
        ...
        {step === 'input' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="p-4 rounded-xl bg-[var(--color-surface2)] border border-[var(--color-border)]">
                <p className="text-xs text-[var(--color-text-muted)] uppercase tracking-wider mb-1">Wallet USDC</p>
            ...
            {/* Network info + Proxy deposit address */}
            {mode === 'transfer' && (
              <div className="space-y-3">
                ...
                {getProxyDepositAddress(user) ? (
                  <div className="p-3 rounded-xl bg-[var(--color-surface2)] border border-[var(--color-border)]">
                    <p className="text-xs text-[var(--color-text-muted)] uppercase tracking-wider mb-1">Your Deposit Wallet (non-custodial)</p>
                    <div className="flex items-center gap-2">
                      <p className="text-xs font-mono text-[var(--color-text)] break-all flex-1">{getProxyDepositAddress(user)}</p>
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(getProxyDepositAddress(user));
                          toast.success('Address copied');
                        }}
                        className="shrink-0 p-1.5 rounded-lg hover:bg-[var(--color-surface3)] text-[var(--color-text-muted)]"
                        title="Copy address"
                      >
                        <Copy className="w-3.5 h-3.5" />
                      </button>
                    </div>
                    <p className="text-xs text-emerald-400 mt-1">✓ Funds go directly to your wallet — not a shared pool</p>
                  </div>
                ) : (
                  <div className="p-3 rounded-xl bg-[var(--color-surface2)] border border-[var(--color-border)]">
                    <p className="text-xs text-[var(--color-text-muted)]">Your deposit wallet is being provisioned. Please refresh the page.</p>
                  </div>
                )}
              </div>
            )}
            ...
            {/* Amount Input */}
            <div>
              <p className="text-sm font-medium text-[var(--color-text)] mb-3">Deposit Amount (USDC)</p>
              <div className="relative">
            ...
            {parsedAmount > 0 && (
              <div className="p-4 rounded-xl bg-[var(--color-surface2)] border border-[var(--color-border)] space-y-2">
                ...
                <div className="border-t border-[var(--color-border)] pt-2 flex justify-between text-sm">
                  ...
                  <span className="font-bold text-emerald-400">{parsedAmount.toFixed(2)} USDC</span>
                </div>
              </div>
            )}
            ...
            >
              ...
              {mode === 'transfer'
                ? `Deposit ${parsedAmount > 0 ? parsedAmount.toFixed(2) : '0'} USDC`
                : 'Claim Deposit'}
            </button>
        ...
        {step === 'processing' && (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[var(--color-text-muted)]">
              Confirming on-chain USDC transfer...
            </p>
            <div className="mt-6 space-y-3 max-w-xs mx-auto">
              {['Sending USDC on Polygon', 'Confirming transaction', 'Crediting your account'].map((s, i) => (
        ...
        {step === 'success' && depositResult && (
          <div className="text-center py-12">
            ...
            <p className="text-lg text-emerald-400 font-bold mb-6">
              +${depositResult.net.toFixed(2)} USDC
            </p>
            ...
            <div className="max-w-xs mx-auto p-4 rounded-xl bg-[var(--color-surface2)] border border-[var(--color-border)] space-y-2 mb-8 text-left">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-emerald-400 font-semibold">${depositResult.net.toFixed(2)} USDC</span>
              </div>
              <div className="flex justify-between text-sm">
