...
const defaultForm = () => ({
  ...
  createOnChain: ONCHAIN_ENABLED, // Always true when on-chain is enabled
  useNegRisk: false,
});
              ...
              {ONCHAIN_ENABLED && (
                <>
                  <div className="flex items-center justify-between pt-3 border-t border-[var(--color-border)]">
                    <div>
                      <label className="text-xs font-medium text-[var(--color-text)]">Deploy on-chain</label>
                      <p className="text-[10px] text-[var(--color-text-muted)]">Creates CTF market on Polygon Amoy</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => set('createOnChain', !form.createOnChain)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--color-gold)] focus:ring-offset-2 focus:ring-offset-[var(--color-surface)] ${form.createOnChain ? 'bg-[var(--color-gold)]' : 'bg-[var(--color-surface2)]'}`}
                    >
                      <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${form.createOnChain ? 'translate-x-5' : 'translate-x-0.5'}`} />
                    </button>
                  </div>
                  ...
                  {form.createOnChain && (
                    <div className="flex items-center justify-between">
                      <div>
                        <label className="text-xs font-medium text-[var(--color-text)]">NegRisk market</label>
                        <p className="text-[10px] text-[var(--color-text-muted)]">Multi-outcome (e.g., World Cup)</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => set('useNegRisk', !form.useNegRisk)}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--color-gold)] focus:ring-offset-2 focus:ring-offset-[var(--color-surface)] ${form.useNegRisk ? 'bg-[var(--color-gold)]' : 'bg-[var(--color-surface2)]'}`}
                      >
                        <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${form.useNegRisk ? 'translate-x-5' : 'translate-x-0.5'}`} />
                      </button>
                    </div>
                  )}
                </>
