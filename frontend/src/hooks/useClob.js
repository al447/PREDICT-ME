...
/**
 * Get the best available EIP-712 signer.
 * Priority: Magic (social login) → injected wallet (MetaMask / Rabby / etc.)
 */
async function getEthersSigner() {
  try {
    ...
    // Magic not configured or user not logged in — fall through to injected wallet
  }
...
export function useCreateOrder() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // maker = proxy (POLY_PROXY or Safe); sigType from user record
...
export function useRealtimeOrderBook(conditionId, tokenId) {
  ...
  return {
    orderBook: connected ? orderBook : restOrderBook, // useOrderBook already returns response.data
    trades,
    connected,
...
/**
 * Get own CLOB activity for a market (real PredictMe user activity, no bot)
 */
export function useClobActivity(conditionId, limit = 20) {
  return useQuery({
    queryKey: ['clob-activity', conditionId, limit],
    queryFn: async () => {
      if (!conditionId) return { activities: [], source: 'predictme' };
      const response = await settlementAPI.get('/clob/activity', {
...
/**
 * Get own CLOB holders for a market (real PredictMe users, no bot)
 */
export function useClobHolders(conditionId, limit = 10) {
  return useQuery({
    queryKey: ['clob-holders', conditionId, limit],
    queryFn: async () => {
      if (!conditionId) return { holders: [], source: 'predictme' };
      const response = await settlementAPI.get('/clob/holders', {
        params: { conditionId, limit },
      });
      return response.data;
    },
