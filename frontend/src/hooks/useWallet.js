...
const useWallet = () => {
  ...
  const signInWithInjectedProvider = async (injected, { referralCode, walletName = 'Wallet' } = {}) => {
    try {
      ...
      const message = `Sign in to PredictMe\nAddress: ${walletAddress}\nTimestamp: ${timestamp}`;
      ...
      console.error(`[${walletName}] Full error:`, err);
      console.error(`[${walletName}] Response data:`, err.response?.data);
      console.error(`[${walletName}] Response status:`, err.response?.status);
